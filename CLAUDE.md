...
## SEARCH TAB NEED LABELS ALSO RENAMED (2026-07-07, follow-on)
Michael caught that the Explore rename didn't reach the Search tab — the need-mix bar/chips, the "Outside 'X' →" toggle, and each result card's colored eyebrow tag were all still showing the old canonical names (e.g. "NAMING THE MACHINE"). Added the same `LABEL`/`NAME()` map (values identical to the geology/universe mockups) to the Search IIFE in `site/template.html`, right after the `NEEDS`/`NCOL` destructure. Four render sites now route through `NAME()`: the need-mix bar segment tooltip, the need-mix chip text (the `data-need` attribute driving the click-filter stays canonical, only the visible label changed), the dominant-need toggle button text, and `cardHTML`'s eyebrow span. Deliberately left canonical everywhere else: the `#not=` URL hash encoding (internal state, not prose), and the "Copy for AI" clipboard text (external LLM synthesis — canonical taxonomy names likely carry more signal there than the terse verbs). Rebuilt via `build_site.py`; confirmed all 7 new labels present in the built `site/index.html` and the inlined script passes `node --check`.
...
- `mockup_discover_universe.html` had NO display map — every render site (anchor labels, legend, focusOn label/message, click-through panel, theme panel, look&feel header) printed the raw canonical key. Added the same `LABEL`/`NAME()` pattern and routed all six sites through it.
- Also moved the globe's own search bar (`#geosearch`/`#geomode` in `mockup_discover_geology.html`) down from `top:16px`/`top:56px` to `top:76px`/`top:116px` — it was sitting at the exact same fixed top-center position as the parent page's tab switcher (`nav` in `site/template.html`, `top:18px`, ~44px tall) and overlapping it.
- Rebuilt via `python3 build_geology.py` + `python3 build_site.py`; both inlined scripts pass `node --check`. WebGL render path not visually verified (no browser in this build env, per the standing caveat on this whole subsystem).
...
- **FIRST LIVE RUN 2026-07-02 (Certified Human round 1):** 40/40 URLs fetched (static 34, playwright 6), **1064 images extracted, 0 hard `failed`** — better than predicted. No code changes needed; setup + both commands ran clean on Python 3.9.6. Fetch-method learnings, all worth encoding for future rounds:
  ...
  - **NEW PATTERN — gallery / grad-show pages flood the corpus.** `1granary.com`'s CSM BA Fashion 2026 page dumped its **entire 853-photo runway gallery** (all unique full-res, correctly passing the junk filter) = 80% of the whole run's 1064 images. For round 1 it was capped MANUALLY to 15 representative photos by trimming that article in a COPY of the output JSON (`output/*_capped.json`). **SUPERSEDED (Michael 2026-07-02): permanent caps are now built into `run.py scrape` — `--max-per-article 15` (first-N in document order, true count kept as `images_found`) and `--target-images 250` (the collection target per code/run; scrape stops early when the pool fills, un-scraped articles stay out of the ledger; shortfall warns and suggests --limit/--days/--urls). Default --limit raised 40→60. Offline tests now 39/39.** Image-rich non-gallery sources this run: radii 25, sightunseen/thisiscolossal/impulse 18, culturedmag 17, sixthtone 11, hyperallergic 9.
...
## SEMIOTIC CODES — Self-Authorship pilot (2026-07-05, 3rd need after Certified Human + Naming the Machine)
Ran the full image_scraper contact-sheet workflow for **Self-Authorship** (`codes_self-authorship.json`). Pilot only — GATE NOT YET PASSED (Michael reviews before scale-out or UI). **Pool:** round 1 = 38 corpus articles / 253 images / 15 sheets; round-2 WebSearch gap-fill = 7 URLs (6 productive) / 69 images; cache re-composited to 20 sheets / 239 slots; ~44 articles with imagery viewed. **5 codes** distributed across the need's Artist↔Algorithm / Roots↔Fame axis:
- **author-among-the-work** (dominant/high) — the self certified by proximity to a visible body of work: artist in studio amid canvases, maker at the worktable, writer framed by the library. Broadest evidence (impulse/hyperallergic/culturedmag/wallpaper/anothermag/aliciakennedy).
- **self-as-the-work** (dominant/high) — persona/body IS the medium: theatrical staged self-portraiture, author=director=subject (Martine Gutierrez, Maya Man, the Cahun/mirror/contact-sheet lineage via round-2 Aperture).
- **heritage-re-signed** (emergent/medium) — inherited form re-authored in a personal register, roots without nostalgia (Roda Medhat Kurdish neon-weave, Vicuña quipu, Spandita Malik/Sikander, "How Asian Is It?", Russell Craig's reclaimed prison-ID). Round-2 Artsy + Wallpaper confirmed multi-region breadth.
- **remix-self** (emergent/medium) — internet-native identity assembled from digital fragments: Y2K glitter, affirmations, jelly-transparency (Maya Man "the internet is air", Polyester). ~4 sources, slightly under bar — flagged.
- **against-the-algorithm** (emergent/**low**) — self as what a model can't generate; the unrepeatable hand/voice. **Deliberately fenced off from Certified Human's provenance/anti-AI frame** (theme-gravity discipline) — here it's ONLY the irreplaceable personal signature, not made-by-humans in general. Anchor = Hauser & Wirth "post-AI painting". **SHORTFALL:** only ~2 clean self-authorship visual sources; most anti-AI imagery is generic editorial illustration. Needs a real round-2 (named artists whose signature IS the subject) before any status/confidence move.
- **Fetch learnings (this need):** clean static/playwright run, 1 hard-thin only (zeitzmocaa.museum exhibition-announcement = 0 imgs, the museum pattern). `outland.art` playwright→0 (JS-gated lazy gallery). **culturedmag.com and hauserwirth.com block WebFetch (403/429)** so quotes for those codes came from captions/observation, not body text — image_refs still verified live (200) via curl. **artsy.net + 032c serve via image proxies** (cloudfront `d7hftxdivxxvm`, Next.js `_next/image`); decoded the 032c ones to direct `cdn.sanity.io` hotlinks, kept Artsy's proxy URL (renders). **impulsemagazine boilerplate trap:** an Ai-Weiwei headshot + social-share icons recur as sidebar furniture across every impulse article — do NOT cite as an article's own imagery (excluded here). Newsletter/wire tier (default.blog, milleworld, aliciakennedy, sixthtone) confirmed text-heavy/thin-image again. All 32 image_refs are live hotlinks (referrerpolicy=no-referrer), spot-checked 200. `GAP_REPORT_self-authorship_round1.md` + `urls_self-authorship_round2.txt` retained; cache/sheets cleaned.
